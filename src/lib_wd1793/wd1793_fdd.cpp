...
int C_Fdd::read(uint8_t type)
{
  ...
  else if (type == snUDI) ok = read_udi();
  else if (type == snHOB) ok = read_hob();
  else if (type == snSCL) ok = read_scl();
  else if (type == snFDI) ok = read_fdi();
  else if (type == snTD0) ok = read_td0();
