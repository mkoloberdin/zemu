...
read_first_byte:	data = trkcache.trkd[rwptr++];
				...
				if (trkcache.hdr[foundid].data[-1] == 0xF8) status |= WDS_RECORDT;
				else status &= ~WDS_RECORDT;
...
char C_Wd1793::notready()
{
	...
	if (!(rqs & DRQ)) return 0; // [rst] было (!wd93_nodelay || !(rqs & DRQ)), но при этом глючит anamnesis
...
void C_Wd1793::out(uint8_t port, uint8_t val, int64_t ttime, int *err)
{
	...
	*err = 0;
	...
	if (port & 0x80) // system
	{
		...
		// [rst] возможно reset долнен происходить по перепаду сигнала (0/1)
		if (!(val & 0x04)) // reset
		{
			status = WDS_NOTRDY;
			rqs = INTRQ;
