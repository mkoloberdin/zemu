...
enum DRIVE_STATE wd1793_get_drive_state(int drive, uint64_t time) {
    ...
    if (!wd.is_drive_spinning(drive, time)) {
        st = DS_INACTIVE;
    } else if (drive != wd.get_current_drive()) {
        st = DS_IDLE;
    } else {
        switch (wd.get_state()) {
            case C_Wd1793::S_WRSEC: // fallthrough
            case C_Wd1793::S_WRITE: // fallthrough
            case C_Wd1793::S_WRTRACK: // fallthrough
            ...
            case C_Wd1793::S_SEEKSTART: // fallthrough
