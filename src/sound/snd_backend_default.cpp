...
void CSndBackendDefault::Init(void) {
    if (alreadyInited) {
        StrikeError("[CSndBackendDefault::Init] Init called twice");
    }

    ZHW_Audio_Spec desired;
    ZHW_Audio_Spec obtained;
    ...
    if (ZHW_Audio_Open(&desired, &obtained) < 0) {
        StrikeError("[CSndBackendDefault::Init] Couldn't open audio: %s", ZHW_Error_Get());
    }
...
void CSndBackendDefault::Write(uint8_t* data, unsigned len) {
    if (!alreadyInited) {
        StrikeError("[CSndBackendDefault::Write] Call Init first");
    }
...
unsigned int CSndBackendDefault::CalcDist(void) {
    return ((dataPtr < audioPtr) ? (audioPtr - dataPtr) : (size - dataPtr + audioPtr));
}
...
void CSndBackendDefault::AudioCallback(void* userData, uint8_t* stream, int len) {
    ...
    if (self->currentPreAgainCnt > 0) {
        self->currentPreAgainCnt--;
    }
