...
{
  Z80EX_BYTE b, b_prev, rep_byte, rep_count;
  unsigned addr;
  int cb;
...
bool load_z80_snap(const char *filename, Z80EX_CONTEXT *cpu, C_MemoryManager &mmgr, C_Border &border)
{
  ...
  else // Z80 v2 or v3
  {
    ...
    header_add = new Z80EX_BYTE[tmp];	// вроду как нет понту проверять header_addr на NULL, ибо stdc++ либа сама кинет эксцэпшн если не получиться выделить память
