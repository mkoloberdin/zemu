...
void C_Mouse::UpdateState(void)
{
	...
	bt = ZHW_Moyse_GetRelativeState(&mx, &my);
	portFBDF += mx / params.mouseDiv;
	portFFDF -= my / params.mouseDiv;

	portFADF = (wheelCnt << 4) | 0x0F;

	if (bt & ZHW_MOUSE_LMASK) portFADF &= ~1;
	if (bt & ZHW_MOUSE_RMASK) portFADF &= ~2;
	if (bt & ZHW_MOUSE_MMASK) portFADF &= ~3;	// middle button works as LMB + RMB
}
